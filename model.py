...
class Proposed_ver3(nn.Module):
    def __init__(self, group, out_channels, eps=1e-5):
        super(Proposed_ver3, self).__init__()
        self.weight = nn.Parameter(torch.ones(1,out_channels,1,1))
        self.bias = nn.Parameter(torch.zeros(1,out_channels,1,1))
        self.group = group
        self.eps = eps
...
class ResNet(nn.Module):
    def __init__(self, num_class=10, layer=56, Method=False, group=1, batch_size=None, Mode=0):
        super(ResNet, self).__init__()
        self.conv1 = nn.Conv2d(3, 16, kernel_size=3, stride=1, padding=1, bias=False)
        self.norm = norm2d(16, group, Method, batch_size=batch_size, width_height=32, use=(Mode==0 or Mode==2))
        ...
        self.layer1.add_module('layer1_0', BasicBlock(in_channels=16, out_channels=16, stride=1, downsample=None, Method=Method, group=group, batch_size=batch_size, width_height=32, use=(Mode==2)))
        for i in range(1,self.n):
            self.layer1.add_module('layer1_%d' % (i), BasicBlock(in_channels=16, out_channels=16, stride=1, downsample=None, Method=Method, group=group, batch_size=batch_size, width_height=32, use=(Mode==2)))

        self.layer2 = nn.Sequential()
        self.layer2.add_module('layer2_0', BasicBlock(in_channels=16, out_channels=32, stride=2, downsample=True, Method=Method, group=group, batch_size=batch_size, width_height=16, use=(Mode==2)))
        for i in range(1,self.n):
            self.layer2.add_module('layer2_%d' % (i), BasicBlock(in_channels=32, out_channels=32, stride=1, downsample=None, Method=Method, group=group, batch_size=batch_size, width_height=16, use=(Mode==2)))

        self.layer3 = nn.Sequential()
        self.layer3.add_module('layer3_0', BasicBlock(in_channels=32, out_channels=64, stride=2, downsample=True, Method=Method, group=group, batch_size=batch_size, width_height=8, use=(Mode==2)))
        for i in range(1,self.n-1):
            self.layer3.add_module('layer3_%d' % (i), BasicBlock(in_channels=64, out_channels=64, stride=1, downsample=None, Method=Method, group=group, batch_size=batch_size, width_height=8, use=(Mode==2)))
        self.layer3.add_module('layer3_%d' % (self.n-1), BasicBlock(in_channels=64, out_channels=64, stride=1, downsample=None, Method=Method, group=group, batch_size=batch_size, width_height=8, use=(Mode==1 or Mode==2)))
